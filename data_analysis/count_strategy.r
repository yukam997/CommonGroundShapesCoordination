...
classify_strategy <- function(pair_choice_seq) {
  strat_table <- c("stable_orange"=0, "alt_purple"=0, "stable_purple"=0, 'both_stable'=0)
  ...
  for (i in (n_rounds-4):(n_rounds-1)) {
    ...
    if (setequal(unique(c(p1_choice, p2_choice,next_p1_choice,next_p2_choice)), c("C", "D"))) {
      if (p1_choice == next_p1_choice & p2_choice == next_p2_choice){strat_table["stable_purple"]=strat_table["stable_purple"]+0.25}
      else if (p1_choice == next_p2_choice & p2_choice == next_p1_choice){strat_table["alt_purple"]=strat_table["alt_purple"]+0.25}
    } 
    else if (setequal(unique(c(p1_choice, p2_choice,next_p1_choice,next_p2_choice)), c("A", "B"))) {
...
for (i in seq_len(nrow(game))) {
  ...
  strategy_count_df <- rbind(strategy_count_df, data.frame(
    ...
    other = 1 - strat["alt_purple"] - strat["stable_orange"] - strat["stable_purple"] - strat["both_stable"],  # Added quotes
    treatmentName = treatment_name
  ))
}
write.csv(strategy_count_df,"./classified_strategies.csv",row.names = FALSE)

experimental_d <- read.csv("./classified_strategies.csv")
View(experimental_d)
...
for (tName in unique(game$treatmentName)) {
  ...
  strategy_counts_exp <- data.frame(
    ...
    count = c(sum(experimental_d_sub$stable_orange), sum(experimental_d_sub$stable_purple), sum(experimental_d_sub$alt_purple), sum(experimental_d_sub$both_stable),sum(experimental_d_sub$other)),
    treatmentName = tName
  )
